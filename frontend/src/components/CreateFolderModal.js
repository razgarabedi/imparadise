...
const CreateFolderModal = ({ isOpen, onClose, onFolderCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await folderService.createFolder(folderName, isPublic);
      onFolderCreated();
      onClose();
      setFolderName('');
      setIsPublic(false);
    } catch (err) {
      setError('Failed to create folder. Folder may already exist or another error occurred.');
    }
  };
                  ...
                  <div className="mt-4">
                    <label className="flex items-center">
                      <input
                        ...
                        className="form-checkbox h-4 w-4 text-accent border-border bg-background rounded"
                      />
