  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8 border-b border-gray-200 dark:border-gray-700 pb-4">
        <h2 className="text-3xl font-bold text-gray-800 dark:text-gray-100">Your Folders</h2>
        <button 
          onClick={() => setIsCreateModalOpen(true)} 
          className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded-md shadow-sm transition-colors duration-300"
        >
          Create New Folder
        </button>
      </div>
      ...
      {folders.length > 0 ? (
          ...
          {folders.map((folder) => (
            <div key={folder.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
              <Link to={`/folders/${folder.id}`} className="block p-6">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{folder.is_public ? 'Public' : 'Private'}</p>
              </Link>
        ...
        <div className="text-center py-16">
          <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-300">No folders yet</h3>
          <p className="text-gray-500 dark:text-gray-400 mt-2">Get started by creating your first folder.</p>
        </div>
      )}
    </div>
  );
