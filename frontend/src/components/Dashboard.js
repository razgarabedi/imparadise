  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8 border-b border-border pb-4">
        <h2 className="text-3xl font-bold text-text">{t('dashboard.title')}</h2>
        <button 
          onClick={() => setIsCreateModalOpen(true)} 
          className="bg-success-emphasis hover:bg-success-emphasis-hover text-white font-bold py-2 px-4 rounded-md flex items-center"
        >
      ...
      {folders.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {folders.map((folder) => (
            <div key={folder.id} className="bg-background rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
              <Link to={`/folders/${folder.id}`} className="block p-6">
                <h3 className="font-bold text-xl text-text truncate">{folder.name}</h3>
                <p className="text-sm text-muted mt-1">{folder.is_public ? t('dashboard.public') : t('dashboard.private')}</p>
              </Link>
              <div className="bg-background-muted px-6 py-3 flex justify-end items-center space-x-2">
                {folder.is_public && (
                  <button onClick={() => handleShareFolder(folder.id)} className="text-muted hover:text-accent p-2 transition-colors duration-300 relative">
                    {copiedFolderId === folder.id ? (
                      <span className="text-sm text-success">{t('dashboard.copied')}</span>
                    ) : (
                ...
                <button onClick={() => openEditModal(folder)} className="text-muted hover:text-accent p-2 transition-colors duration-300">
                ...
                <button onClick={() => openDeleteModal(folder)} className="text-muted hover:text-danger p-2 transition-colors duration-300">
        ...
        <div className="text-center py-16">
          <h3 className="text-xl font-semibold text-text">{t('dashboard.no_folders_title')}</h3>
          <p className="text-muted mt-2">{t('dashboard.no_folders_subtitle')}</p>
        </div>
      )}
    </div>
  );
